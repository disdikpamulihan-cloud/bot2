...
def train_bot2_model():
    logging.info("🧠 Bot 2: Pelatihan AI 6 Fitur Dimulai...")
    df = fetch_expert_data(count=5000)
    if df.empty or len(df) < 400: return False
    ...
    df_feat = pd.DataFrame({
        ...
        'ATR': atr, 'RSI': rsi, 'Momentum': momentum, 'BodySize': body_size
    }).dropna()

    X, y = [], []
    for i in range(200, len(df_feat) - 3):
        row = df_feat.iloc[i]
        # PENTING: Tepat 6 Fitur
        features = [
            float(row['ATR']), 
            float(row['BodySize']), 
    ...
    clf1 = RandomForestClassifier(n_estimators=500, max_depth=12, random_state=42, class_weight='balanced')
    clf2 = GradientBoostingClassifier(n_estimators=300, learning_rate=0.02, max_depth=5, random_state=42)

    mastermind_model = VotingClassifier(estimators=[('rf', clf1), ('gb', clf2)], voting='soft')
    mastermind_model.fit(X_train, y_train)

    score = mastermind_model.score(X_test, y_test)
    logging.info(f"✨ Model Dilatih! Akurasi test: {score * 100:.2f}%")

    joblib.dump(mastermind_model, "model_bot2.pkl")
    send_telegram_alert(f"🧠 *AI Model Updated!* Akurasi: `{score * 100:.2f}%`")
    return True
